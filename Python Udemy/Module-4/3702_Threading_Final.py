# '3701_Image_Extract.py' works fine without threading, but
#   freezes when object removes from frame. But this code
#   with threading didn't works properly. So, check it.
...
# (Threading => Because image freezes when object exits frame.
#               So, it allows function to run in background
#               without freezing main video frame.)
...
def clean_folder(): # clean 'image' folder.
    print("clean_folder function started") # Good with threading to avoid confusion.
    images = glob.glob("images/*.png")
    for image in images:
        os.remove(image)
    print("clean_folder function ended")   # Good with threading to avoid confusion.
...
while True:
    ...
    # i.e. when object just removes from the frame.
        ...
        email_thread.start() # Function execution
...
clean_thread.start() # Function execution
# (↑) I put this function outside-loop, because by using threading
#      programs run in background in ambiguous-order i.e. may be
#      files will delete before email was sent.