// Problem-2: Palindrome Queries
//             Calculate no. of different substrings 
//             which are Palindrome.
// Ans=> See Slide-1, Page-32 to 33.
//        TC = O(n).
...
struct Hash{
    ...
    Hash(string s){
        ...
        for(int i=0; i < s.size(); i++){
            Hashes[i + 1] = {(1ll * Hashes[i].first + (s[i] - 'a' + 1) * Pow[i].first) % MOD,
                             (1ll * Hashes[i].second + (s[i] - 'a' + 1) * Pow[i].second) % MOD};
