...
struct SegTree {
	...
	void make_update(int index, int x, int y) {  // changed (←)
		Update new_update = Update(x, y); // changed (←)
...
struct Node1 {
	...
	void merge(Node1 &l, Node1 &r) { // Merge two child nodes
		val = (l.val ^ r.val);  // changed (←)
	}
};

struct Update1 {
    int x, y; // changed (←)
	Update1(int x1, int y1) { // changed (←)
		x = x1; // changed (←)
		y = y1; // changed (←)
	}
	void apply(Node1 &a) { // apply update to given node
		a.val = ((a.val * x) + y); // changed (←)
	}
};
// Segment-Tree Template (↑)
