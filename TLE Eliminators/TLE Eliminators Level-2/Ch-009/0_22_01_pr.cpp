// Wrong.
// Here, by line 51, 57 and 65 are giving me Wrong OUTPUT of '0'.
...
signed main(){
    ...
    int n; cin >> n;
    vector<long long> v(n); // Here, by '(n)' it is giving me Wrong OUTPUT.
    for(int i=0; i<n; i++){ 
        long long temp; cin >> temp;
        v.push_back(temp);
    }

    vector<long long> prefix_Sum1(n+1);  // Here, by '(n)' it is giving me Wrong OUTPUT.
    ...
    sort(v.begin(), v.end());
    vector<long long> prefix_Sum2(n+1);  // Here, by '(n)' it is giving me Wrong OUTPUT.
